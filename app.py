    ...
    for i in range(height):
        for j in range(width):
            ...
            if (small_image[i, j]==[0,0,0]).all():
                big_image[position[0]+i, position[1]+j] = [0,0,0]
...
class ImageApp:
    def __init__(self, master):
        ...
        self.x_label = tk.Label(self.top_frame, text="X:")
        self.y_label = tk.Label(self.top_frame, text="Y:")
    ...
    def choose_color(self):
        ...
        pil_image = Image.fromarray(cv2.cvtColor(cv2.convertScaleAbs(PAGE_GEN), cv2.COLOR_BGR2RGB)) 
        pil_image = pil_image.resize((507, 720))
        photo = ImageTk.PhotoImage(pil_image)
        self.pic1.config(image=photo)
        self.photo1 = photo
